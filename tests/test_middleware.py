...
async def test_inject_error_500(client):
    resp = await client.get("/mgmt/info?inject_error=500")
...
async def test_inject_error_429(client):
    resp = await client.get("/mgmt/info?inject_error=429")
    assert resp.status_code == 429


@pytest.mark.anyio
async def test_inject_validation_error(client):
    resp = await client.get("/mgmt/info?inject_error=validation_error")
    assert resp.status_code == 422


@pytest.mark.anyio
async def test_inject_custom_error(client):
    resp = await client.get("/mgmt/info?inject_error=custom:boom")
    assert resp.status_code == 500
...
async def test_delay_ms(client):
    ...
    resp = await client.get("/mgmt/ready?delay_ms=100")
